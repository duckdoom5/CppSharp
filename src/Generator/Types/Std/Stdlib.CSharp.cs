...
namespace CppSharp.Types.Std.CSharp
{
    ...
    public class Optional : TypeMap
    {
        ...
        public override void MarshalToNative(MarshalContext ctx)
        {
            ctx.Before.WriteLine("throw new System.NotImplementedException(\"Setting std::optional is currently not supported\");");
            ...
            var assign = optional.Methods.First(m => m.Name == "OperatorEqual");
            if (ctx.MarshalKind == MarshalKind.NativeField)
            {
                string var;
                if (ctx.ReturnVarName.LastIndexOf('.') > ctx.ReturnVarName.LastIndexOf("->"))
