...
namespace CppSharp::CppParser {
    ...
    public:
        ...
        [[nodiscard]] AST::Declaration* VisitNamedDecl(const clang::NamedDecl* ND);
        ...
        [[nodiscard]] AST::Declaration* VisitRecordDecl(const clang::RecordDecl* RD);
    ...
    private:
        ...
        void writeBareSourceLocation(clang::SourceLocation Loc, bool IsSpelling);
        ...
        void writeTemplateDeclSpecialization(const SpecializationDecl* SD, bool DumpExplicitInst, bool DumpRefOnly)
        {
            ...
            for (const auto* RedeclWithBadType : SD->redecls()) {
                ...
                case clang::TSK_ImplicitInstantiation:
                    if (DumpRefOnly)
                        NodeVisitor.AddChild([=] { NodeVisitor.writeBareDeclRef(Redecl); });
                    else
                        Visit(Redecl);
                    DumpedAny = true;
                    break;
            ...
            if (!DumpedAny)
                NodeVisitor.AddChild([=] { NodeVisitor.writeBareDeclRef(SD); });
        }
